...
def query_repos() -> requests.Response:
    ...
    assert response.status_code == 200, f"API connection to [{url}] failed\n\twith status code: {response.status_code}"
...
def display_data(repo_dicts: list[dict]) -> None:
    ...
    for repo_dict in repo_dicts:
        assert repo_dict['stargazers_count'] > 10000, f"{repo_dict['name']}: {repo_dict['stargazers_count']} stars!"
