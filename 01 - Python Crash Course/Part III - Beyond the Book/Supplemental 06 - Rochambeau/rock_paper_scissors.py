...
class RockPaperScissors:
    ...
    def __init__(self, use_strategy = None, debug_mode = False):
        ...
        # If the calling program did not set a strategy, use the "win-stay, lose-shift" strategy 50% of games
        self.use_strategy = self.coin_flip() if use_strategy == None else use_strategy
    ...
    def strategic_choice(self):
        ...
        if self.last_result[0] == "w":
            # A player after losing has a higher likelihood of shifting to the action that would have beaten the computer's last play
            # So the computer's strategic action is to play whatever would beat whatever would beat the computer's last action
            computer_action = self.beats[self.beats[self.last_computer_action]]
        elif self.last_result[0] == "l":
            # A player after winning has a higher likelihood of staying with the same action on the next turn
            # So the computer's strategic action is to play whatever would beat the player's last action
            computer_action = self.beats[self.last_player_action]
        else:
            # On the first play of the game or following a tie, choose an action at random
            computer_action = self.random_choice()
        return computer_action

    def turn(self, player_action):
        """Play a turn"""
        # Determine the computer's action based on whether or not strategy is active
        computer_action = self.strategic_choice() if self.use_strategy else self.random_choice()
    ...
    def play(self):
        ...
        while True:
            ...
            # Obtain an action from the player and verify that it is in the list of available actions
            while player_action not in self.available_actions:
                player_action = input("Enter your choice: ([r]ock, [p]aper, [s]cissors, [q]uit\n> ")
                if player_action == "" or player_action == None: continue
                player_action = player_action[0].lower()
            
            # If the player's response starts with "q", end the game
            if player_action == "q": break

            # Otherwise, play the turn using the player's chosen action
            self.turn(player_action)

if __name__ == "__main__":
    """Main guard the file to ensure it is the one running"""
