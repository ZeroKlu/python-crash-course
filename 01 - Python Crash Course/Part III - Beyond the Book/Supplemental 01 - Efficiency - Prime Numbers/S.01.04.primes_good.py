...
# We are still able to nearly double our efficiency from the already efficient algorithm
...
# In my test, this ran for 0.07 seconds
def is_prime(n):
    ...
    if n == 2:
        return True
    # Even numbers higher than two cannot be prime, so let's eliminate them
    if n % 2 == 0:
        return False
    # The maximum number you need to divide by is the square root of the number
    max_div = int(n ** 0.5)
    # Now we can just test odd numbers up to the square root of n
